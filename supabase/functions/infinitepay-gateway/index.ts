...
function normalizeBrazilPhone(value: unknown, requireMobile = false): string {
  ...
  if (d.length === 11 && subscriber[0] !== "9") {
    throw new Error("Celular inválido: depois do DDD, o número deve começar com 9.");
  }
  if (d.length === 10 && requireMobile) {
    throw new Error("Para WhatsApp de celular, informe DDD + 9 dígitos.");
  }
...
async function createCashPayment(body: any) {
  ...
  const { data, error } = await db.rpc("start_cash_payment", {
    p_name: name,
    p_whatsapp: whatsapp,
    p_numbers: numbers,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (body?.action === "create_cash_payment") {
      return json(await createCashPayment(body));
    }
